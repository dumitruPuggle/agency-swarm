...
class Thread:
    ...
    def get_completion(
        ...
        print(
            f"THREAD:[ {
                sender_name} -> {recipient_agent.name} ]: URL {self.thread_url}"
        )
    ...
    def execute_tool(
        ...
        if not tool:
            return (
                f"Error: Function {tool_call.function.name} not found. Available functions: {
        ...
        try:
            ...
            for tool_name in [name for name, _ in tool_outputs_and_names]:
                if tool_name == tool_name and (
                    ...
                    return (
                        f"Error: Function {
                            tool_name} is already called. You can only call this function once at a time. Please wait for the previous call to finish before calling it again.",
                        False,
                    )
            ...
            if tool_name.startswith("SendMessage"):
                if tool.recipient.value in self._called_recepients:
                    return (
                        f"Error: Agent {
                            tool.recipient.value} has already been called. You can only call each agent once at a time. Please wait for the previous call to finish before calling it again.",
                        False,
                    )
    ...
    def _get_sync_async_tool_calls(self, tool_calls, recipient_agent):
        ...
        for tool_call in tool_calls:
            ...
            if tool is None:
                logger.warning(
                    f"Tool {tool_call.function.name} not found in agent {
                        recipient_agent.name}. Skipping."
                )
                continue

            if (
                hasattr(tool.ToolConfig,
                        "async_mode") and tool.ToolConfig.async_mode
